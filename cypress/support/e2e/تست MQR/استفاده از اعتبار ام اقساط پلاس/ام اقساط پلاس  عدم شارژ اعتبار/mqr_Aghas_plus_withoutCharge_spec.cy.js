describe("mqr test", () => {
  ...
  it("M_Aghsat_Plus without charge credit test", () => {
    ...
    cy.wait(350);
    cy.get(
      '.cardtwo > :nth-child(1) > [style="display: flex; text-align: center;"] > [style="display: flex; width: 100%; padding: 2% 1%; border-radius: 10px; align-items: center; justify-content: center; margin: auto; flex-direction: column;"] > :nth-child(1) > .nav-link > .sc-gtcAbF'
    ).click({ force: true });
    cy.wait(350);
      ...
      cy.then(() => {
        cy.getAllLocalStorage().then((result) => {
          token = JSON.parse(result["https://stage1.qhami.com"].mresalatPwa)
            .token.access_token; //دریافت توکن
        });
      }).then(() => {
        cy.wait(350);
          ...
          cy.request({
            ...
            cy.then(() => {
              response.body.creditFacilityHesanResponseModels.map((item) => {
                if (item.facilityModel) {
                  //درصورت true بودن اعتبار قابل استفاده میباشد.
                  item.creditFacilityHesanListModelList.map(
                    (creditFacility) => {
                ...
                cy.get(".div5-in-card-peyment-aghsat > div:nth-child(2)") //چک کردن مقدار باقی مانده
                  ...
                  .then((valueStr) => {
                    ...
                    if (value == 0) {
                      ...
                      //صفر  نشدن مبلغ باقی مانده
                      if (!hasCreditPlus) {
                        // درصورت عدم وجود اعتبار عملیات متوقف شود
                        return false;
                      }
              ...
              cy.wait(350);
              cy.get(".mqrCreditPaymen-amount").type(amountFinal); //وارد کردن مبلغ قابل پرداخت
              cy.wait(350);
              ...
              cy.then(() => {
                ...
                if (checkCredit) {
                  ...
                  cy.window().then((win) => {
                    ...
                    ); //دریافت اطلاعات شخص لاگین شده
                    cy.wait(500);
                    cy.get(".Input_cnfPayment_modal_one")
                      .type(getUserInfo.nationalCode) //وارد کردن کدملی
                      .invoke("val")
                      .then((inputValue) => {
